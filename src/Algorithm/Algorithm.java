...
public class Algorithm {
    ...
    public Algorithm(ArrayList<String> desiredCourses){
        courses = new ArrayList<course>();
        getCourses(desiredCourses);
        ArrayList<Section> sched = new ArrayList<Section>();
        schedule = sched;
    }

    private void getCourses(ArrayList<String> desiredCourses){
        Query query = new Query();
        try {
            for (String str : desiredCourses) {
                ArrayList<String> sections = query.getCourseSections(str);  // AL<String>
                ArrayList<Section> Sections = new ArrayList<Section>();
                for (String sec : sections) {
                    Sections.add(query.makeSection(sec));
                }
                courses.add(new course(Sections, str));
            }
    ...
    private void opt(ArrayList<Section> schedule, int courseIndex, double score){
        ...
        for(Section s : courseSections) {
            ...
            opt(sched, courseIndex + 1, score + s.getRating());
        }
    }
